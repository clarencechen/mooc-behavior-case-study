...
my_verticals = Vertical_Output(dataset_names[1])
...
#my_keras_model.create_basic_transformer_model(lrate=2e-3, layers=4, embed_dim=128, seq_len=sequence_max_len, \
#   model_load_path='transformer_weights_{}'.format(COURSE_NAMES[1]))
...
new_verticals = Vertical_Output(dataset_names[0])
...
my_keras_model = MOOC_Keras_Model(new_verticals.pre_index_data.vertical_index.max())
my_keras_model.create_finetune_transformer_model('transformer_weights_{}'.format(COURSE_NAMES[1]), lrate=2e-4, layers=4, embed_dim=128, seq_len=sequence_max_len, freeze_layers=True)

my_keras_model.transformer_model_fit(train_x, train_y, val_x, val_y, epoch_limit=20, batch_size=64, \
    model_save_path='transformer_weights_finetuned_{}'.format(COURSE_NAMES[0]), tensorboard_log_path='tensorboard_logs')
